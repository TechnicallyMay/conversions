...
func NewList(fromQty float64, fromName string, toQty float64, toName string) *unitNode {
    ...
    logger.Debug(logPrefix + "First conversion in list, %v -%v-> %v", smallerName, smallerToLargerScale, largerName)
...
func (n *unitNode) insertUnitAtTargetScale(newUnitName string, targetScale float64) bool {
    ...
    for curr.Next != nil && currScale * curr.ScaleToNext < targetScale {
        logger.Debug(logPrefix + "curr, next %v, %v", *curr.name, *curr.Next.name)
        currScale *= curr.ScaleToNext
        curr = curr.Next
    }

    proportion := targetScale / currScale
    newScaleToNext := math.Max(1, curr.ScaleToNext / proportion)
    newUnit := &unitNode{name: &newUnitName, ScaleToNext: newScaleToNext, Next: curr.Next}
    curr.ScaleToNext = proportion
    curr.Next = newUnit

    logger.Debug(logPrefix + "TargetScale: %v, CurrentScale: %v", targetScale, currScale)
    logger.Debug(logPrefix + "Adding conversion %v -%v-> %v.", *curr.name, proportion, newUnitName)
